          ...
          {[
            ...
            <div key={index} className="flex items-center justify-between text-[10px] sm:text-xs">
              <div className="flex items-center gap-1 sm:gap-2">
                <div
                  className={`w-1.5 h-1.5 sm:w-2 sm:h-2 rounded-full ${category.type === 'fixed' ? 'bg-blue-500' : 'bg-green-500'
                    }`}
                />
                <span className="truncate">{category.name}</span>
              </div>
              <div className="text-right shrink-0">
                <div className={`font-medium ${category.spent > category.budgeted ? 'text-red-600' : 'text-foreground'
                  }`}>
