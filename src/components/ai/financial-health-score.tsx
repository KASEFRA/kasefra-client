    ...
    <Card className="premium-card hover-lift border-0 shadow-md">
      <CardHeader className="pb-3 sm:pb-4 px-3 sm:px-6">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 sm:gap-0">
          <div className="min-w-0 flex-1">
            <CardTitle className="flex items-center gap-2 text-sm sm:text-base lg:text-lg font-bold">
              <div className="icon-container bg-primary/10 p-1 sm:p-1.5 rounded">
                <TrendingUp className="h-3 w-3 sm:h-4 sm:w-4 lg:h-5 lg:w-5 text-primary" />
              </div>
              <span className="truncate">Financial Health</span>
              {getTrendIcon()}
            </CardTitle>
            <CardDescription className="text-xs sm:text-sm lg:text-base">AI-powered wellness analysis</CardDescription>
          </div>
          <div className="text-center sm:text-right shrink-0">
            <div className="text-2xl sm:text-3xl font-bold text-primary">{overall}</div>
      ...
      <CardContent className="space-y-4 sm:space-y-6 px-3 sm:px-6">
        ...
        <div>
          ...
          <Progress value={overall} className="h-1.5 sm:h-2" />
        </div>

        {/* Component Breakdown */}
        <div className="space-y-3 sm:space-y-4">
          <h4 className="font-semibold text-xs sm:text-sm">Score Breakdown</h4>

          {/* Savings Rate */}
          <div className="space-y-1 sm:space-y-2">
            <div className="flex items-center justify-between gap-2">
              <span className="text-xs sm:text-sm truncate">Savings Rate</span>
              <div className="flex items-center gap-1 sm:gap-2 shrink-0">
                <span className="text-xs sm:text-sm font-medium">{components.savingsRate.value}%</span>
                <Badge size="sm" className={`${getStatusColor(components.savingsRate.status)} text-[10px] sm:text-xs`}>
          ...
          <div className="space-y-1 sm:space-y-2">
            <div className="flex items-center justify-between gap-2">
              <span className="text-xs sm:text-sm truncate">Debt-to-Income Ratio</span>
              <div className="flex items-center gap-1 sm:gap-2 shrink-0">
                <span className="text-xs sm:text-sm font-medium">{components.debtRatio.value}%</span>
                <Badge size="sm" className={`${getStatusColor(components.debtRatio.status)} text-[10px] sm:text-xs`}>
          ...
          <div className="space-y-1 sm:space-y-2">
            <div className="flex items-center justify-between gap-2">
              <span className="text-xs sm:text-sm truncate">Emergency Fund</span>
              <div className="flex items-center gap-1 sm:gap-2 shrink-0">
                <span className="text-xs sm:text-sm font-medium">{components.emergencyFund.months} months</span>
                <Badge size="sm" className={`${getStatusColor(components.emergencyFund.status)} text-[10px] sm:text-xs`}>
        ...
        <div className="space-y-2 sm:space-y-3">
          <h4 className="font-semibold text-xs sm:text-sm">AI Recommendations</h4>
          <div className="space-y-1 sm:space-y-2">
            {recommendations.slice(0, 3).map((rec, index) => (
              <div key={index} className="flex items-start space-x-2 p-2 rounded-lg bg-muted/50">
                ...
                <p className="text-xs sm:text-sm text-muted-foreground leading-relaxed">{rec}</p>
              </div>
            ))}
          </div>
        </div>
